...
void DBGTask(void* argument)
{
    ...
    debug.Add_ValueCommander("H", &control.Tar_L0_L);
    debug.Add_ValueCommander("V", &control.Tar_d_Yaw, &control.Tar_d_x);
    while (1)
    {
        debug.Run_Debug();
        printf("theta:%f, d_theta:%f, x:%f, d_x:%f, phi:%f, d_phi:%f, T:%f, Tp:%f\r\n",
