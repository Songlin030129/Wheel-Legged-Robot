...
void vmc_calc_kf(VMC* _vmc)
{
    ...
    _vmc->lBD = sqrt((_vmc->XD - _vmc->XB) * (_vmc->XD - _vmc->XB) +
                     (_vmc->YD - _vmc->YB) * (_vmc->YD - _vmc->YB));
    ...
    _vmc->phi2 =
        2 *
        atan2f((_vmc->B0 + sqrt(_vmc->A0 * _vmc->A0 + _vmc->B0 * _vmc->B0 - _vmc->C0 * _vmc->C0)),
               _vmc->A0 + _vmc->C0);
    _vmc->phi3 = atan2f(_vmc->YB - _vmc->YD + _vmc->l2 * arm_sin_f32(_vmc->phi2),
                        _vmc->XB - _vmc->XD + _vmc->l2 * arm_cos_f32(_vmc->phi2));
    // C点直角坐标
    ...
    // C点极坐标
    _vmc->L0 =
        sqrt((_vmc->XC - _vmc->l5 / 2.0f) * (_vmc->XC - _vmc->l5 / 2.0f) + _vmc->YC * _vmc->YC);
    ...
    _vmc->d_phi0 = (_vmc->phi0 - _vmc->last_phi0) /
                   _vmc->dt;  // 计算phi0变化率，d_phi0用于计算lqr需要的d_theta
...
{
    _vmc->j11 =
        (_vmc->l1 * arm_sin_f32(_vmc->phi0 - _vmc->phi3) * arm_sin_f32(_vmc->phi1 - _vmc->phi2)) /
        arm_sin_f32(_vmc->phi3 - _vmc->phi2);
    _vmc->j12 =
        (_vmc->l1 * arm_cos_f32(_vmc->phi0 - _vmc->phi3) * arm_sin_f32(_vmc->phi1 - _vmc->phi2)) /
        (_vmc->L0 * arm_sin_f32(_vmc->phi3 - _vmc->phi2));
    _vmc->j21 =
        (_vmc->l4 * arm_sin_f32(_vmc->phi0 - _vmc->phi2) * arm_sin_f32(_vmc->phi3 - _vmc->phi4)) /
        arm_sin_f32(_vmc->phi3 - _vmc->phi2);
    _vmc->j22 =
        (_vmc->l4 * arm_cos_f32(_vmc->phi0 - _vmc->phi2) * arm_sin_f32(_vmc->phi3 - _vmc->phi4)) /
        (_vmc->L0 * arm_sin_f32(_vmc->phi3 - _vmc->phi2));

    _vmc->torque_set[0] =
        _vmc->j11 * _vmc->F0 +
        _vmc->j12 * _vmc->Tp;  // 得到RightFront的输出轴期望力矩，F0为五连杆机构末端沿腿的推力
    _vmc->torque_set[1] =
        _vmc->j21 * _vmc->F0 +
        _vmc->j22 * _vmc->Tp;  // 得到RightBack的输出轴期望力矩，Tp为沿中心轴的力矩
}

void vmc_calc_fn(VMC* _vmc, float _feedback_t1, float _feedback_t2)
{
    _vmc->Tp_reverse = (_feedback_t2 - _feedback_t1 * (_vmc->j21 / _vmc->j11)) * _vmc->j11 /
                       (_vmc->j11 * _vmc->j22 - _vmc->j21 * _vmc->j12);
    ...
    float dd_Zm =
        (INS.Accel[1] * arm_sin_f32(-_vmc->phi) + INS.Accel[2] * arm_cos_f32(-_vmc->phi)) -
        _GRAVITY;
    float dd_Zw = dd_Zm - _vmc->dd_L0 * cos_theta + 2 * _vmc->d_L0 * _vmc->d_theta * sin_theta +
                  _vmc->L0 * _vmc->dd_theta * sin_theta +
                  _vmc->L0 * _vmc->d_theta * _vmc->d_theta * cos_theta;
