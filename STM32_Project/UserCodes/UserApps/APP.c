...
void APP()
{
    ...
    xReturn = xTaskCreate(DBGTask, "DBGTask", 512, NULL, osPriorityNormal, NULL);
    if (xReturn == pdTRUE)
        printf("DBG Task Create Success!\r\n");
    else
        printf("DBG Task Create Fail\r\n");
    xReturn = xTaskCreate(INSTask, "INSTask", 512, NULL, osPriorityNormal, NULL);
    if (xReturn == pdTRUE)
        printf("INS Task Create Success!\r\n");
    else
        printf("INS Task Create Fail\r\n");
    xReturn = xTaskCreate(ControlTask, "ControlTask", 512, NULL, osPriorityNormal, NULL);
    if (xReturn == pdTRUE)
